...
echoheader( "<i class=\"fa fa-home position-left\"></i><span class=\"text-semibold\">Модерация отзывов</span>","<a href=\"$PHP_SELF?mod=mfo\">Каталог МФО 7.0</a> / Модерация отзывов" );
	
$entries = "";

$db->query( "SELECT " . PREFIX . "_mfo_comments.id, " . PREFIX . "_mfo_comments.mfo_id, " . PREFIX . "_mfo_comments.otziv, " . PREFIX . "_mfo_comments.tit_otziv, ". PREFIX . "_mfo_comments.otziv, " . PREFIX . "_mfo_comments.name_otziv, " . PREFIX . "_mfo_comments.date, " . PREFIX . "_mfo_comments.author, " . PREFIX . "_mfo_comments.text, " . PREFIX . "_mfo_post.title, " . PREFIX . "_mfo_post.category FROM " . PREFIX . "_mfo_comments LEFT JOIN " . PREFIX . "_mfo_post ON " . PREFIX . "_mfo_comments.mfo_id=" . PREFIX . "_mfo_post.id WHERE " . PREFIX . "_mfo_comments.approve = '0' ORDER BY " . PREFIX . "_mfo_comments.date DESC" );
...
<div id='table-comm-{$row['id']}' class="panel panel-default">
  ...
  <div class="panel-footer">
    <button id="save-button-{$row['id']}" onclick="public_comm('{$row['id']}', '{$row['mfo_id']}' , '{$row['otziv']}'); return false;" type="button" class="btn bg-teal btn-sm btn-raised position-left"><i class="fa fa-check-square-o position-left"></i>{$lang['bb_b_approve']}</button>
	...
	<button onclick="DeleteComments('{$row['id']}', '{$row['mfo_id']}'); return false;" type="button" class="btn bg-danger btn-sm btn-raised"><i class="fa fa-trash-o position-left"></i>{$lang['edit_dnews']}</button>
...
function public_comm( c_id, mfo_id, otziv ) {

	ShowLoading('');

	$.post('engine/ajax/controller.php?mod=modermfo', { id: c_id, mfo_id:mfo_id, otziv: otziv, action: "commentspublic", user_hash: '{$dle_login_hash}' }, function(data){
...
function DeleteComments(id, mfo_id) {

    DLEconfirm( 'Вы уверены что хотите удалить данный отзыв?', '{$lang['p_confirm']}', function () {

		ShowLoading('');
	
		$.get("engine/ajax/controller.php?mod=delotzivmfo", { id: id, mfo_id: mfo_id }, function(data){
