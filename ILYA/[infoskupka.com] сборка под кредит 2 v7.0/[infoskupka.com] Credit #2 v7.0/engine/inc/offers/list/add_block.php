...
//-------------------------------------------------====
echoheader( "<i class=\"fa fa-home position-left\"></i><span class=\"text-semibold\">Добавление блока</span>","<a href=\"$PHP_SELF?mod=offers\">Каталог офферов 7.0</a> / <a href=\"$PHP_SELF?mod=offers&do=blocks\">Блоки</a>" );
...
if( isset( $_POST['submit'] ) )
	{
		...
		$db->query( "INSERT INTO ".PREFIX."_offers_options (`type`,`options`,`sort`) VALUES ('blocks','{$Options}','1')" );
...
<div style=\"padding: 20px;\">
		...
		else
	{
		...
		$postType = makeDropDown( array( "default" => "Все офферы", "color" => "Офферы - Рекомендуем" ), "post_type", "default" );
		$AllowCats = $Offers->SelectCategory( array( "all" ), "", "", "", true );
		$CatsType = makeDropDown( array( "only" => "Только выделенные категории", "parented" => "С подкатегориями" ), "cats_type", "onky" );
		$cache = 120;
		$max_post = 10;
		$template = "default";
