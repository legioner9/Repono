...
$_SESSION['admin_referrer'] = "?mod=offers&amp;do=otziv&amp;action=edit";
...
if( $action == "dodelete" AND $id) {
	...
	$db->query( "UPDATE " . PREFIX . "offers_post SET comm_num='0' WHERE id ='{$id}'" );
	...
	msg( "success", $lang['mass_head'], $lang['mass_delokc'], "?mod=offers&amp;do=otziv&amp;action=&id={$id}" );

} elseif( $action == "edit" ) {

	if ( $id ) $where = "offers_id = '{$id}' AND "; else $where = "";
	...
	echoheader( "<i class=\"fa fa-home position-left\"></i><span class=\"text-semibold\">Управление отзывами</span>","<a href=\"$PHP_SELF?mod=offers\">Каталог офферов 7.0</a> / Управление отзывами" );
	
	$entries = "";

	$result_count = $db->super_query( "SELECT COUNT(*) as count FROM " . PREFIX . "_offers_comments WHERE {$where}approve='1'" );

	$db->query( "SELECT " . PREFIX . "_offers_comments.id, offers_id, " . PREFIX . "_offers_comments.tit_otziv, ". PREFIX . "_offers_comments.otziv, " . PREFIX . "_offers_comments.name_otziv, " . PREFIX . "_offers_comments.date, " . PREFIX . "_offers_comments.id, " . PREFIX . "_offers_comments.text, " . PREFIX . "_offers_comments.author, " . PREFIX . "_offers_post.title, " . PREFIX . "_offers_post.alt_name, " . PREFIX . "_offers_post.category FROM " . PREFIX . "_offers_comments LEFT JOIN " . PREFIX . "_offers_post ON " . PREFIX . "_offers_comments.offers_id=" . PREFIX . "_offers_post.id WHERE {$where}" . PREFIX . "_offers_comments.approve = '1' ORDER BY " . PREFIX . "_offers_comments.date DESC LIMIT $start_from,$news_per_page" );
...
<div id='table-comm-{$row['id']}' class="panel panel-default">
  ...
  <div class="panel-footer">
	...
	<button onclick="DeleteComments('{$row['id']}', '{$row['offers_id']}', '{$row['otziv']}'); return false;" type="button" class="btn bg-danger btn-sm btn-raised"><i class="fa fa-trash-o position-left"></i>{$lang['edit_dnews']}</button>
	<span class="pull-right" style="margin-top: 4px;"><i class="fa fa-clock-o position-left"></i>{$date}</span>
  </div>
</div>
<input type="hidden" name="offers_id[{$row['id']}]" value="{$row['offers_id']}">
HTML;
	...
	$db->free();
		...
		if( $start_from > 0 ) {
			...
			$npp_nav .= "<li><a href=\"?mod=offers&amp;do=otziv&amp;action=&id={$id}&start_from={$previous}\" title=\"{$lang['edit_prev']}\">&lt;&lt;</a></li>";
		}
		...
		if( $result_count['count'] > $news_per_page ) {
			...
			if( $enpages_count <= 10 ) {
				...
				for($j = 1; $j <= $enpages_count; $j ++) {
					...
					if( $enpages_start_from != $start_from ) {
						...
						$enpages .= "<li><a href=\"?mod=offers&amp;do=otziv&amp;action=&id={$id}&start_from={$enpages_start_from}\">$j</a></li>";
				...
				if( $start > 2 ) {
					...
					$enpages .= "<li><a href=\"?mod=offers&amp;do=otziv&amp;action=&id={$id}&start_from=0\">1</a></li> <li><span>...</span></li>";
				...
				$enpages .= "<li><span>...</span></li><li><a href=\"?mod=offers&amp;do=otziv&amp;action=&id={$id}&start_from={$enpages_start_from}\">$enpages_count</a></li>";
			...
			if( $result_count['count'] > $i ) {
				...
				$npp_nav .= "<li><a href=\"?mod=offers&amp;do=otziv&amp;action=&id={$id}&start_from={$i}\" title=\"{$lang['edit_next']}\">&gt;&gt;</a></li>";
			}
...
function DeleteComments(id, offers_id, otziv) {

    DLEconfirm( 'Вы уверены что хотите удалить данный отзыв?', '{$lang['p_confirm']}', function () {

		ShowLoading('');
	
		$.get("engine/ajax/controller.php?mod=delotzivoffers", { id: id, offers_id: offers_id, otziv: otziv }, function(r){
...
<div class="panel panel-flat">
	<div class="panel-heading">
		<span class="text-semibold" style="font-size:15px;">Все отзывы о офферах</span> 
	</div>
</div>
{$entries}
{$npp_nav}
