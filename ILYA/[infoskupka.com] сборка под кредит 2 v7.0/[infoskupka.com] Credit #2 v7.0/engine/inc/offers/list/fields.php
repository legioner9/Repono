...
echoheader( "<i class=\"fa fa-home position-left\"></i><span class=\"text-semibold\">Дополнительные поля</span>","<a href=\"$PHP_SELF?mod=offers\">Каталог офферов 7.0</a> / <a href=\"$PHP_SELF?mod=offers&do=fields\">Дополнительные поля</a>" );


if( $Offers->Field === false ) $Offers->StartField( true );

$list = "";	
if( count( $Offers->Field->DB ) > 0 )
	{
		...
		foreach( $Offers->Field->DB as $row )
			{
				...
				$sort = intval( $row['11'] );
				$type = $Offers->Field->TypeName[ $row[12] ];
...
$list .= <<<HTML
		...
		<tr id="del_Field_{$id}" height="30px" onmouseover="this.bgColor='#F1F7FE'" onmouseout="this.bgColor='#FFFFFF'">
			...
			<td>
				<a href="$PHP_SELF?mod=offers&do=edit_fields&show_action=edit&id={$id}"><b>{$name}</b>   -   Тег для шаблона: {Offersfield:{$alt_name}}</a>
			</td>
...
<form name="fields" method="post" action="" onsubmit="SetFormSubmit( 'fields' ); return false;">
	<table class="table table-striped table-xs table-hover">
		...
		<a href="$PHP_SELF?mod=offers&do=edit_fields&show_action=add" class="btn bg-teal btn-sm btn-raised position-left legitRipple" />Добавить поле</a>
		</ul>
		</div>
