...
echoheader( "<i class=\"fa fa-home position-left\"></i><span class=\"text-semibold\">Добавление категории</span>","<a href=\"$PHP_SELF?mod=offers\">Каталог офферов 7.0</a> / <a href=\"$PHP_SELF?mod=offers&do=category\">Категории</a>" );
...
if( isset( $_POST['submit'] ) )
	{
		...
		foreach( $Offers->DBIndex['category'] as $alt_name => $id )
			{
		...
		$db->query( "INSERT INTO ".PREFIX."_offers_options (`type`,`options`,`sort`) VALUES ('category','{$Options}','{$sort}')" );
		$Offers->CleareAllCache();
		
		echo ThisClose();
		echo "<div class=\"panel-body\"><div style=\"color: green; float: left; margin: 20px 40px 20px 0;\">Категория добавлена <a class=\"btn bg-teal btn-sm btn-raised position-left legitRipple\" href=\"$PHP_SELF?mod=offers&amp;do=add_category\">Добавить еще</a><a class=\"btn bg-teal btn-sm btn-raised position-left legitRipple\" href=\"$PHP_SELF?mod=offers&amp;do=category\">Список категорий</a></div></div>";
	}
		else
	{
		$SelectCategory = $Offers->SelectCategory();
