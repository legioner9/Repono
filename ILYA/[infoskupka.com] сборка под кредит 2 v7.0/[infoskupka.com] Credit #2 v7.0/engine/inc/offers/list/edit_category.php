...
echoheader( "<i class=\"fa fa-home position-left\"></i><span class=\"text-semibold\">Редактирование категории</span>","<a href=\"$PHP_SELF?mod=offers\">Каталог офферов 7.0</a> / <a href=\"$PHP_SELF?mod=offers&do=category\">Категории</a>" );
				...
				if( isset( $_POST['submit'] ) )
					{
						...
						$Options['count_post'] = $Offers->DB['category'][ $id ]['count_post'];
						...
						$db->query( "UPDATE ".PREFIX."_offers_options SET `options`='{$Options}',`sort`='{$sort}' WHERE `id`='{$id}' LIMIT 1" );
...
<div style=\"padding: 20px;\">
	<a href=\"$PHP_SELF?mod=offers&do=category\" class=\"btn bg-slate-600 btn-sm btn-raised legitRipple\" style=\"margin-right:5px;\">Список категорий<a/>
</div>";
					}
						else
					{
						$name = htmlspecialchars( stripslashes( $Offers->DB['category'][ $id ]['name'] ), ENT_QUOTES, $config['charset'] );
						$alt_name = htmlspecialchars( stripslashes( $Offers->DB['category'][ $id ]['alt_name'] ), ENT_QUOTES, $config['charset'] );
						$template = htmlspecialchars( stripslashes( $Offers->DB['category'][ $id ]['template'] ), ENT_QUOTES, $config['charset'] );
						$title_h = htmlspecialchars( stripslashes( $Offers->DB['category'][ $id ]['title_h'] ), ENT_QUOTES, $config['charset'] );
						$description = htmlspecialchars( stripslashes( $Offers->DB['category'][ $id ]['description'] ), ENT_QUOTES, $config['charset'] );
						$opisanie = $Offers->DB['category'][ $id ]['opisanie'];
						$opisanie_up = $Offers->DB['category'][ $id ]['opisanie_up'];
						$keywords = htmlspecialchars( stripslashes( $Offers->DB['category'][ $id ]['keywords'] ), ENT_QUOTES, $config['charset'] );
						$sort = intval( $Offers->DB['category'][ $id ]['sort'] );
						$MainCategory = intval( $Offers->DB['category'][ $id ]['cat'] );
						$SelectCategory = $Offers->SelectCategory( $MainCategory, $id );
redactor();	
