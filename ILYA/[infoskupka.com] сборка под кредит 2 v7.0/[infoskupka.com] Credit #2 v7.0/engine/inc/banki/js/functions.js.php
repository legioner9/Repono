...
<script language="javascript" type="text/javascript">
	...
	function ajaxBanki( resultID, fileUrl, dataUrl, statusID, imgUrl, endImgHtml, endFunction ){
		
		if( !statusID ) statusID = "";
		if( !imgUrl ) imgUrl = "status.gif";
		if( statusID != "" ) $( "#" + statusID ).html( "<img src=\"engine/inc/banki/style/images/" + imgUrl + "\" border=\"0\" alt=\"Загрузка\" align=\"absmiddle\" />" );
	...
	if( !UpdateUrl ) var UpdateUrl = "mainbanki";
	var UpdateButton = "<img src=\"engine/inc/banki/style/images/ajax.png\" alt=\"Обновить\" title=\"Обновить\" style=\"cursor: pointer;\" onclick=\"UpdateList(); return false;\" border=\"0\" />";
	function UpdateList( url, otherData, result, preloder ){
		
		if( !url ) url = UpdateUrl;
		if( url == "mainbanki" && !otherData ) otherData = UpdateUrlMainData;
		if( preloder )
			{
				document.getElementById( preloder ).innerHTML = "<div style=\"padding: 30px; text-align: center;\"><img src=\"engine/inc/banki/style/images/ajax_preloader.gif\" alt=\"Загрузка\" border=\"0\" /></div>";
			}
		
		document.getElementById( "UpdateStatus" ).innerHTML = "<img src=\"engine/inc/banki/style/images/ajax.gif\" alt=\"Загрузка\" border=\"0\" />";
		
		var idEcho = ( result ) ? result : url;
		ajaxBanki( idEcho, "/engine/ajax/banki/admin/list.update.php", otherData + "&action=" + url, "UpdateStatus", "ajax.gif", "", function( data, statusID, resultID ){
			$( "#" + resultID ).html( data );
			document.getElementById( "UpdateStatus" ).innerHTML = "<img src=\"engine/inc/banki/style/images/ajax.png\" alt=\"Обновить\" title=\"Обновить\" style=\"cursor: pointer;\" onclick=\"UpdateList(); return false;\" border=\"0\" />";
	...
	function CleareCache(){
		...
		$.ajax({
			url: "/engine/ajax/banki/admin/other.options.php",
	...
	function SetStatusAlbum( type, id ){
		...
		document.getElementById( "album_status_" + id ).innerHTML = "<img src=\"engine/inc/banki/style/images/mini-ajax.gif\" alt=\"Загрузка\" border=\"0\" />";
	...
	function DelAlbum( id ){
		...
		if( Quest )
			{
				document.getElementById( "DelAlbum_" + id ).innerHTML = "<img src=\"engine/inc/banki/style/images/ajax_preloader.gif\" alt=\"Удаление\" border=\"0\" />";
	...
	function formSubmit( nameForm, file, IdResult, img, imgId, onCompletion, onCompletionValue ){		
		...
		document.getElementById( imgId ).innerHTML = "<img src=\"engine/inc/banki/style/images/" + img + "\" border=\"0\" alt=\"Загрузка\" />";
		...
		$.ajax({
			url: "/engine/ajax/banki/admin/" + file,
			data: Url,
			success: function( data ){
				$( "#" + IdResult ).html( data );
	...
	function AddBlocks(){
		...
		document.getElementById( "LightWindow-content" ).innerHTML = "<div style=\"padding: 15px; text-align: center;\"><img src=\"engine/inc/banki/style/images/preloader.gif\" alt=\"Загрузка...\" border=\"0\" /></div>";
				
		$.ajax({
			url: "/engine/ajax/banki/admin/blocks/add.blocks.php",
			data: "",
			success: function( data ){
	...
	function EditBlocks( id ){
		...
		$.ajax({
			url: "/engine/ajax/banki/admin/blocks/edit.blocks.php",
			data: "id=" + id,
			success: function( data ){
				$( "#LightWindow-content" ).html( data );
			},
	...
	function CodeBlocks( id ){
		prompt( "Вставьте данный код в шаблон:", "{include file=\"engine/modules/banki/block.php?id=" + id + "\"}" );
	}
	...
	function DelBlocks( id ){
		...
		if( Quest )
			{
				document.getElementById( "StatusIDBlocks_" + id ).innerHTML = "<img src=\"engine/inc/banki/style/images/mini-ajax.gif\" alt=\"...\" border=\"0\" />";
				
				$.ajax({
					url: "/engine/ajax/banki/admin/blocks/del.blocks.php",
					data: "id=" + id,
					success: function( data ){
	...
	function DelBankiType( id ){
		
		var Quest = confirm( "Вы действительно желаете удалить данный тип предложений?" );
		if( Quest )
			{
				document.getElementById( "StatusIDBankiType_" + id ).innerHTML = "<img src=\"engine/inc/banki/style/images/mini-ajax.gif\" alt=\"...\" border=\"0\" />";
				
				$.ajax({
					url: "/engine/ajax/banki/admin/banki/del.banki_type.php",
					data: "id=" + id,
					success: function( data ){
						$( "#del_BankiType_" + id ).html( data );
						var Element = document.getElementById( "del_BankiType_" + id );
						Element.parentNode.removeChild( Element );
					},
					dataType: "html",
					type: "POST"
	...
	function AddCurrency(){
		...
		$.ajax({
			url: "/engine/ajax/banki/admin/currency/add.currency.php",
			data: "",
			success: function( data ){
				$( "#LightWindow-content" ).html( data );
			},
	...
	function EditCurrency( id ){
		...
		$.ajax({
			url: "/engine/ajax/banki/admin/currency/edit.currency.php",
			data: "id=" + id,
			success: function( data ){
				$( "#LightWindow-content" ).html( data );
			},
	...
	function DelCurrency( id ){
		...
		if( Quest )
			{
				document.getElementById( "StatusIDCurrency_" + id ).innerHTML = "<img src=\"engine/inc/banki/style/images/mini-ajax.gif\" alt=\"...\" border=\"0\" />";
				
				$.ajax({
					url: "/engine/ajax/banki/admin/currency/del.currency.php",
					data: "id=" + id,
					success: function( data ){
	...
	function AddCountry(){
		...
		$.ajax({
			url: "/engine/ajax/banki/admin/country/add.country.php",
			data: "",
			success: function( data ){
				$( "#LightWindow-content" ).html( data );
			},
	...
	function EditCountry( id ){
		...
		$.ajax({
			url: "/engine/ajax/banki/admin/country/edit.country.php",
			data: "id=" + id,
			success: function( data ){
				$( "#LightWindow-content" ).html( data );
			},
	...
	function DelCountry( id ){
		...
		if( Quest )
			{
				document.getElementById( "StatusCountryId_" + id ).innerHTML = "<img src=\"engine/inc/banki/style/images/mini-ajax.gif\" alt=\"...\" border=\"0\" />";
				
				$.ajax({
					url: "/engine/ajax/banki/admin/country/del.country.php",
					data: "id=" + id,
					success: function( data ){
	...
	function AddCategory(){
		...
		$.ajax({
			url: "/engine/ajax/banki/admin/category/add.category.php",
			data: "",
			success: function( data ){
				$( "#LightWindow-content" ).html( data );
			},
	...
	function EditCategory( id ){
		...
		$.ajax({
			url: "/engine/ajax/banki/admin/category/edit.category.php",
			data: "id=" + id,
			success: function( data ){
				$( "#LightWindow-content" ).html( data );
			},
	...
	function AddField(){
		...
		var OpenWindow = window.open( "{$PHP_SELF}?mod=banki&action=field&show_action=add", "AddField", "toolbar=0, location=0, status=0, left=300, top=100, menubar=0, scrollbars=yes, resizable=0, width=700, height=405" );
		OpenWindow.focus();
	}
	
	function EditField( id ){
		
		var OpenWindow = window.open( "{$PHP_SELF}?mod=banki&action=field&show_action=edit&id=" + id, "EditField_" + id, "toolbar=0, location=0, status=0, left=300, top=100, menubar=0, scrollbars=yes, resizable=0, width=700, height=405" );
		OpenWindow.focus();
	...
	function TagField( id ){
		...
		var OpenWindow = window.open( "{$PHP_SELF}?mod=banki&action=field&show_action=tag&id=" + id, "TagField_" + id, "toolbar=0, location=0, status=0, left=300, top=150, menubar=0, scrollbars=yes, resizable=0, width=700, height=265" );
		OpenWindow.focus();
	...
	function DelField( id ){
		...
		if( Quest )
			{
				document.getElementById( "StatusIDField_" + id ).innerHTML = "<img src=\"engine/inc/banki/style/images/mini-ajax.gif\" alt=\"...\" border=\"0\" />";
								
				$.ajax({
					url: "/engine/ajax/banki/admin/field/del.field.php",
					data: "id=" + id,
					success: function( data ){
	...
	function DelBankiPost( id ){
		
		var Quest = confirm( "Вы действительно желаете удалить данный оффер со всеми его отзывами?" );
		if( Quest )
			{
				document.getElementById( "StatusIDBankiPost_" + id ).innerHTML = "<img src=\"engine/inc/banki/style/images/mini-ajax.gif\" alt=\"...\" border=\"0\" />";
				
				$.ajax({
					url: "/engine/ajax/banki/admin/banki_post/del.banki_post.php",
					data: "id=" + id,
					success: function( data ){
						$( "#del_BankiPost_" + id ).html( data );
						var Element = document.getElementById( "del_BankiPost_" + id );
						Element.parentNode.removeChild( Element );
					},
					dataType: "html",
					type: "POST"
	...
	function DelCategory( id ){
		...
		if( Quest )
			{
				document.getElementById( "StatusCategory_" + id ).innerHTML = "<img src=\"engine/inc/banki/style/images/mini-ajax.gif\" alt=\"...\" border=\"0\" />";
				
				$.ajax({
					url: "/engine/ajax/banki/admin/category/del.category.php",
					data: "id=" + id,
					success: function( data ){
	...
	function EditBankiPost( id ){
		
		var OpenWindow = window.open( "{$PHP_SELF}?mod=banki&action=post_edit&id=" + id + "", "EditPost_" + id + "", "toolbar=0, location=0, status=0, left=200, top=50, menubar=0, scrollbars=yes, resizable=0, width=900, height=605" );
		OpenWindow.focus();
	}
	...
	function CategorySelect(){
		BankiTypeSetCategory();
		BankiFieldsSetCategory();
	}
	
	function BankiTypeSetCategory(){
		
		var Select = "";
		var SelectCategory = document.getElementById( "BankiSelectCategory" ).value;
		var SelectType = document.getElementById( "BankiTypeSelect" ).value;
		for( var i = 0; i < BankiTypeCat[ SelectCategory ].length; i++ )
			{
				var id = BankiTypeCat[ SelectCategory ][ i ];
				var name = BankiTypeName[ id ];
				var selected = SelectType == id ? " selected=\"selected\"" : "";
				Select += "<option value=\"" + id + "\"" + selected + ">" + name + "</option>";
			}
		
		Select = "<select name=\"banki_type\" id=\"BankiTypeSelect\">" + Select + "</select>";
		document.getElementById( "bankiType" ).innerHTML = Select;
	}
	
	function BankiFieldsSetCategory(){
	
		var SelectCategory = document.getElementById( "BankiSelectCategory" ).value;
		var field_search = document.getElementById( "BankiFields" );
		...
		for( var i = 0; i < count_field; i++ )
			{
				if( field_id[ i ] != null )
					{
						if( /BankiField_[0-9]/i.exec( field_id[ i ].id ) != null )
							{
								field_id[ i ].style['display'] = "none";
								var this_field_id = field_id[ i ].id.match( /[0-9]{1,50}/ );
								
								if( BankiFields[ SelectCategory ] != null )
									{
										for( var c = 0; c < BankiFields[ SelectCategory ].length; c++ )
											{
												if( this_field_id == BankiFields[ SelectCategory ][ c ] )
													{
								...
								if( field_id[ i ].style['display'] != "" && BankiFields['all'] != null )
									{
										for( var c = 0; c < BankiFields['all'].length; c++ )
											{
												if( this_field_id == BankiFields['all'][ c ] )
													{
														field_id[ i ].style['display'] = "";
													}
											}
...
<script type="text/javascript" src="engine/inc/banki/js/ajaxupload.3.5.js"></script>
<script language="JavaScript" type="text/javascript">
	...
	function NewBtnToUpload( Btn, Status, Album ){
		$(function(){
			...
			new AjaxUpload( btnUpload, {
				action: "engine/ajax/banki/admin/upload.php",
				name: 	"Filedata",
	...
	//				upload_name : $("[name=upload_name]").val(),
				...
				onSubmit: function( file, ext ){
					...
					status.html( "<img src=\"engine/inc/banki/style/images/preloader.gif\" alt=\"Загрузка...\" border=\"0\" />" );
				},
