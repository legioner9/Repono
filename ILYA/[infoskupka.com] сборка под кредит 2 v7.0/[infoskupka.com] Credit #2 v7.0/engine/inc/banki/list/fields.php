...
echoheader( "<i class=\"fa fa-home position-left\"></i><span class=\"text-semibold\">Дополнительные поля</span>","<a href=\"$PHP_SELF?mod=banki\">Каталог банков 7.0</a> / <a href=\"$PHP_SELF?mod=banki&do=fields\">Дополнительные поля</a>" );


if( $Banki->Field === false ) $Banki->StartField( true );

$list = "";	
if( count( $Banki->Field->DB ) > 0 )
	{
		...
		foreach( $Banki->Field->DB as $row )
			{
				...
				$sort = intval( $row['11'] );
				$type = $Banki->Field->TypeName[ $row[12] ];
...
$list .= <<<HTML
		...
		<tr id="del_Field_{$id}" height="30px" onmouseover="this.bgColor='#F1F7FE'" onmouseout="this.bgColor='#FFFFFF'">
			...
			<td>
				<a href="$PHP_SELF?mod=banki&do=edit_fields&show_action=edit&id={$id}"><b>{$name}</b>   -   Тег для шаблона: {Bankifield:{$alt_name}}</a>
			</td>
...
<form name="fields" method="post" action="" onsubmit="SetFormSubmit( 'fields' ); return false;">
	<table class="table table-striped table-xs table-hover">
		...
		<a href="$PHP_SELF?mod=banki&do=edit_fields&show_action=add" class="btn bg-teal btn-sm btn-raised position-left legitRipple" />Добавить поле</a>
		</ul>
		</div>
