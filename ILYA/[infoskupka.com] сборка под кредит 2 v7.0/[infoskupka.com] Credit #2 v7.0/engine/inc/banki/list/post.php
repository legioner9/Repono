...
echoheader( "<i class=\"fa fa-home position-left\"></i><span class=\"text-semibold\">Список банков</span>","<a href=\"$PHP_SELF?mod=banki\">Каталог банков 7.0</a> / <a href=\"$PHP_SELF?mod=banki&do=post\">Список банков</a>" );
...
$SelectCategory = $Banki->SelectCategory( $SCategory );


echo <<<HTML
<div class="panel panel-default">		
	<div class="panel-heading">Поиск банков</div>
<div id="main">
<form method="post" action="" class="form-horizontal" name="SearchBankis">
...
if( $Banki->Config['region_on'] == "on" ) echo <<<HTML
...
<div class="panel-footer hidden-xs">
		<div class="pull-left">
			<input type="submit" class="btn bg-teal btn-raised position-left legitRipple" name="submit" value="Начать поиск" class="edit" />
					...
					<input type="hidden" name="sort" id="banki_hidden_sort" value="" />
					 <span id="status_search_banki"></span>
		</div>
...
$temp = $db->super_query( "SELECT COUNT(*) as count FROM ".PREFIX."_banki_post WHERE id!='' {$sql}" ); 
$posts = $temp['count'];
if( $posts > 0 )
	{
		...
		if( !empty( $_REQUEST['sort'] ) )
			{
				$order_banki = totranslit( $_REQUEST['sort'] );
				$array_sort = array(
				  "id_d" => array( "id", true ),
				  "id_k" => array( "id", false ),
				...
				foreach( $array_sort as $name_sort => $value )
					{    
						if( $name_sort == $order_banki )
							{
								$new_sort = $value[0];
								$desc_sort = $value[1];
								if( $desc_sort ) $desc_sort = "DESC";
								$sql_order = "ORDER BY `{$new_sort}` {$desc_sort}";
								echo "<script language=\"Javascript\">document.getElementById( \"banki_hidden_sort\" ).value=\"{$order_banki}\";</script>";
							}
					}
			}
				else
			{
				...
				$sql_order = "ORDER BY `rate` DESC";
				$order_banki = totranslit( $_REQUEST['sort'] );
			}
		
		$list = "";
		$result = $db->query( "SELECT * FROM ".PREFIX."_banki_post WHERE id!='' {$sql} {$sql_order} {$limit}" );
				...
				while( $row = $db->get_row( $result ) )
					{
						...
						$photos = $row['photos'] != "" ? "<img src=\"engine/inc/banki/style/images/photo.png\" alt=\"Имеются фотографии\" title=\"Имеются фотографии\" align=\"absmiddle\" />" : "";
						...
						$EchoCat = ( $category && $Banki->DB['category'][ $category ] ) ? htmlspecialchars( stripslashes( $Banki->DB['category'][ $category ]['title_h'] ), ENT_QUOTES, $config['charset'] ) : "---";
...
$list .= <<<HTML
		...
		<tr id="del_BankiPost_{$id}">
			<td width="70px" align="center" id="StatusIDBankiPost_{$id}">{$id}</td>
			<td>
				{$super_vip}
				{$vip}
				{$photos}
				<a href="?mod=banki&do=edit_post&id={$id}">{$title}</a> <a href="{$LinkPost}" target="_blank" style="float:right;">На сайте <i class="fa fa-play" style="font-size: 10px;"></i></a>
			...
			<td width="100px" align="center">
				<div class="btn-group">
					...
					<ul class="dropdown-menu text-left">
						<li><a href="?mod=banki&do=edit_post&id={$id}"><i class="icon-desktop"></i> Редактировать</a></li>
						<li class="divider"></li>
						<li><a onclick="DelBankiPost( '{$id}' ); return false;" href="#"><i class="icon-trash"></i> удалить</a></li>
					</ul>
				</div>
			</td>
			<td width="30px">
				<input type="checkbox" name="CheckBankiPost[]" value="{$id}" />
			</td>
		</tr>
...
$n_url = "$PHP_SELF?mod=banki&do=post&{$RequestData}";
$ajax_navigation = "UpdateList( 'mainbanki', '{$RequestData}{page}{$order_banki}' );";
$navigation = Banki_Navigation( "{$n_url}{$order_banki}", "{$n_url}{$order_banki}", $total, $page, $ajax_navigation );
...
<form name="main" method="post" action="" onsubmit="SetFormSubmit( 'main' ); return false;">
	...
	<script language="javascript" type="text/javascript">
		var UpdateUrlMainData = "{$RequestData}{$page}&sort={$order_banki}";
	</script>
