...
echoheader( "<i class=\"fa fa-home position-left\"></i><span class=\"text-semibold\">Модерация отзывов</span>","<a href=\"$PHP_SELF?mod=banki\">Каталог банков</a> / Модерация отзывов" );
	
$entries = "";

$db->query( "SELECT " . PREFIX . "_banki_comments.id, " . PREFIX . "_banki_comments.banki_id, " . PREFIX . "_banki_comments.otziv, " . PREFIX . "_banki_comments.tit_otziv, ". PREFIX . "_banki_comments.otziv, " . PREFIX . "_banki_comments.name_otziv, " . PREFIX . "_banki_comments.date, " . PREFIX . "_banki_comments.author, " . PREFIX . "_banki_comments.text, " . PREFIX . "_banki_post.title, " . PREFIX . "_banki_post.category FROM " . PREFIX . "_banki_comments LEFT JOIN " . PREFIX . "_banki_post ON " . PREFIX . "_banki_comments.banki_id=" . PREFIX . "_banki_post.id WHERE " . PREFIX . "_banki_comments.approve = '0' ORDER BY " . PREFIX . "_banki_comments.date DESC" );
...
<div id='table-comm-{$row['id']}' class="panel panel-default">
  ...
  <div class="panel-footer">
    <button id="save-button-{$row['id']}" onclick="public_comm('{$row['id']}', '{$row['banki_id']}' , '{$row['otziv']}'); return false;" type="button" class="btn bg-teal btn-sm btn-raised position-left"><i class="fa fa-check-square-o position-left"></i>{$lang['bb_b_approve']}</button>
	...
	<button onclick="DeleteComments('{$row['id']}', '{$row['banki_id']}'); return false;" type="button" class="btn bg-danger btn-sm btn-raised"><i class="fa fa-trash-o position-left"></i>{$lang['edit_dnews']}</button>
...
function public_comm( c_id, banki_id, otziv ) {

	ShowLoading('');

	$.post('engine/ajax/controller.php?mod=moderbanki', { id: c_id, banki_id:banki_id, otziv: otziv, action: "commentspublic", user_hash: '{$dle_login_hash}' }, function(data){
...
function DeleteComments(id, banki_id) {

    DLEconfirm( 'Вы уверены что хотите удалить данный отзыв?', '{$lang['p_confirm']}', function () {

		ShowLoading('');
	
		$.get("engine/ajax/controller.php?mod=delotzivbank", { id: id, banki_id: banki_id }, function(data){
