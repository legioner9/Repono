...
echoheader( "<i class=\"fa fa-home position-left\"></i><span class=\"text-semibold\">Добавление категории</span>","<a href=\"$PHP_SELF?mod=banki\">Каталог банков 7.0</a> / <a href=\"$PHP_SELF?mod=banki&do=category\">Категории</a>" );
...
if( isset( $_POST['submit'] ) )
	{
		...
		foreach( $Banki->DBIndex['category'] as $alt_name => $id )
			{
		...
		$db->query( "INSERT INTO ".PREFIX."_banki_options (`type`,`options`,`sort`) VALUES ('category','{$Options}','{$sort}')" );
		$Banki->CleareAllCache();
		
		echo ThisClose();
		echo "<div class=\"panel-body\"><div style=\"color: green; float: left; margin: 40px 40px 40px 0;\">Категория добавлена <a style=s\"float: left; href=\"$PHP_SELF?mod=banki&amp;do=category\">Список категорий</a></div></div>";
	}
		else
	{
		$SelectCategory = $Banki->SelectCategory();
