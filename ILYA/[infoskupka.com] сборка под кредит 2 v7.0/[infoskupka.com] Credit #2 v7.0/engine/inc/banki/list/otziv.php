...
$_SESSION['admin_referrer'] = "?mod=banki&amp;do=otziv&amp;action=edit";
...
if( $action == "dodelete" AND $id) {
	...
	$db->query( "UPDATE " . PREFIX . "banki_post SET comm_num='0' WHERE id ='{$id}'" );
	...
	msg( "success", $lang['mass_head'], $lang['mass_delokc'], "?mod=banki&amp;do=otziv&amp;action=&id={$id}" );

} elseif( $action == "edit" ) {

	if ( $id ) $where = "banki_id = '{$id}' AND "; else $where = "";
	...
	echoheader( "<i class=\"fa fa-home position-left\"></i><span class=\"text-semibold\">Управление отзывами</span>","<a href=\"$PHP_SELF?mod=banki\">Каталог банков 7.0</a> / Управление отзывами" );
	
	$entries = "";

	$result_count = $db->super_query( "SELECT COUNT(*) as count FROM " . PREFIX . "_banki_comments WHERE {$where}approve='1'" );

	$db->query( "SELECT " . PREFIX . "_banki_comments.id, banki_id, " . PREFIX . "_banki_comments.tit_otziv, ". PREFIX . "_banki_comments.otziv, " . PREFIX . "_banki_comments.name_otziv, " . PREFIX . "_banki_comments.date, " . PREFIX . "_banki_comments.id, " . PREFIX . "_banki_comments.text, " . PREFIX . "_banki_comments.author, " . PREFIX . "_banki_post.title, " . PREFIX . "_banki_post.alt_name, " . PREFIX . "_banki_post.category FROM " . PREFIX . "_banki_comments LEFT JOIN " . PREFIX . "_banki_post ON " . PREFIX . "_banki_comments.banki_id=" . PREFIX . "_banki_post.id WHERE {$where}" . PREFIX . "_banki_comments.approve = '1' ORDER BY " . PREFIX . "_banki_comments.date DESC LIMIT $start_from,$news_per_page" );
	...
	while ( $row = $db->get_array() ) {
		...
		$banki_id =  stripslashes( $row['banki_id'] );
...
<div id='table-comm-{$row['id']}' class="panel panel-default">
  <div class="panel-heading">
    <span class="label label-info position-left">Логин:</span><strong class="position-left">{$row['author']}</strong><span class="label label-info position-left">ФИО:</span> <strong class="position-left">{$name_otziv}</strong> <span class="label label-info position-left">Банк:</span> <strong class="position-left">{$news_title}</strong> <span class="label label-info position-left">Оценка:</span> <strong class="position-left">{$otziv}</strong>
  </div>
  ...
  <div class="panel-footer">
	...
	<button onclick="DeleteComments('{$row['id']}', '{$row['banki_id']}', '{$row['otziv']}'); return false;" type="button" class="btn bg-danger btn-sm btn-raised"><i class="fa fa-trash-o position-left"></i>{$lang['edit_dnews']}</button>
	<span class="pull-right" style="margin-top: 4px;"><i class="fa fa-clock-o position-left"></i>{$date}</span>
  </div>
</div>
<input type="hidden" name="banki_id[{$row['id']}]" value="{$row['banki_id']}">
HTML;
	...
	$db->free();
		...
		if( $start_from > 0 ) {
			...
			$npp_nav .= "<li><a href=\"?mod=banki&amp;do=otziv&amp;action=&id={$id}&start_from={$previous}\" title=\"{$lang['edit_prev']}\">&lt;&lt;</a></li>";
		}
		...
		if( $result_count['count'] > $news_per_page ) {
			...
			if( $enpages_count <= 10 ) {
				...
				for($j = 1; $j <= $enpages_count; $j ++) {
					...
					if( $enpages_start_from != $start_from ) {
						...
						$enpages .= "<li><a href=\"?mod=banki&amp;do=otziv&amp;action=&id={$id}&start_from={$enpages_start_from}\">$j</a></li>";
				...
				if( $start > 2 ) {
					...
					$enpages .= "<li><a href=\"?mod=banki&amp;do=otziv&amp;action=&id={$id}&start_from=0\">1</a></li> <li><span>...</span></li>";
				...
				$enpages .= "<li><span>...</span></li><li><a href=\"?mod=banki&amp;do=otziv&amp;action=&id={$id}&start_from={$enpages_start_from}\">$enpages_count</a></li>";
			...
			if( $result_count['count'] > $i ) {
				...
				$npp_nav .= "<li><a href=\"?mod=banki&amp;do=otziv&amp;action=&id={$id}&start_from={$i}\" title=\"{$lang['edit_next']}\">&gt;&gt;</a></li>";
			}
...
function ajax_comm_edit( c_id )
{
	...
	$.get("engine/ajax/controller.php?mod=editotziv", { id: c_id, area: 'news', action: "edit" }, function(data){
...
function ajax_save_comm_edit( c_id, area )
{
	...
	$.post("engine/ajax/controller.php?mod=editotziv", { id: c_id, comm_txt: comm_txt, area: area, action: "save", user_hash: "{$dle_login_hash}" }, function(data){
...
function DeleteComments(id, banki_id, otziv) {

    DLEconfirm( 'Вы уверены что хотите удалить данный отзыв?', '{$lang['p_confirm']}', function () {

		ShowLoading('');
	
		$.get("engine/ajax/controller.php?mod=delotzivbank", { id: id, banki_id: banki_id, otziv: otziv }, function(r){
...
<div class="panel panel-flat">
	<div class="panel-heading">
		<span class="text-semibold" style="font-size:15px;">Все отзывы о банках</span> 
	</div>
</div>
{$entries}
{$npp_nav}
