...
echoheader( "<i class=\"fa fa-home position-left\"></i><span class=\"text-semibold\">Блоки</span>","<a href=\"$PHP_SELF?mod=banki\">Каталог банков 7.0</a> / <a href=\"$PHP_SELF?mod=banki&do=blocks\">Блоки</a>" );

$list = "";	
if( count( $Banki->DB['blocks'] ) > 0 )
	{
		foreach( $Banki->DB['blocks'] as $key => $row )
			{
				$id = $row['id'];
...
$list .= <<<HTML
			...
			<td align="center"><a href="$PHP_SELF?mod=banki&do=edit_block&id={$id}" >{$title}</a></td>
			...
			<td width="360px" align="center">
				{include file="engine/modules/banki/block.php?id={$id}"}
...
<form name="blocks" method="post" action="" onsubmit="SetFormSubmit( 'blocks' ); return false;">
	...
	<div class="panel-footer hidden-xs">
		...
		<a  class="btn bg-teal btn-sm btn-raised position-right legitRipple" href="$PHP_SELF?mod=banki&do=add_block" />Добавить блок</a>
		</ul>
		</div>
